...
class AdvancedCacheService {
  ...
  private findOldestEntry(entries: [string, CacheEntry][]): { key: string; timestamp: number } | null {
    if (entries.length === 0) return null
    
    let oldest = entries[0]
    
    for (let i = 1; i < entries.length; i++) {
      if (entries[i][1].timestamp < oldest[1].timestamp) {
        oldest = entries[i]
      }
    }
  ...
  private findNewestEntry(entries: [string, CacheEntry][]): { key: string; timestamp: number } | null {
    if (entries.length === 0) return null
    
    let newest = entries[0]
    
    for (let i = 1; i < entries.length; i++) {
      if (entries[i][1].timestamp > newest[1].timestamp) {
        newest = entries[i]
      }
    }
    
    return { key: newest[0], timestamp: newest[1].timestamp }
  }

  private findMostAccessedEntry(entries: [string, CacheEntry][]): { key: string; accessCount: number } | null {
    if (entries.length === 0) return null
    
    let mostAccessed = entries[0]
    
    for (let i = 1; i < entries.length; i++) {
      if (entries[i][1].accessCount > mostAccessed[1].accessCount) {
        mostAccessed = entries[i]
      }
    }
  ...
  private findLeastAccessedEntry(entries: [string, CacheEntry][]): { key: string; accessCount: number } | null {
    if (entries.length === 0) return null
    
    let leastAccessed = entries[0]
    
    for (let i = 1; i < entries.length; i++) {
      if (entries[i][1].accessCount < leastAccessed[1].accessCount) {
        leastAccessed = entries[i]
      }
    }
