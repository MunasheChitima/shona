...
const achievements = {
  ...
  'vocabulary_master': {
    ...
    icon: '📚',
    xpReward: 75,
    condition: (user) => user.vocabularyLearned >= 100
  },
  'cultural_explorer': {
    ...
    icon: '🌍',
    xpReward: 50,
    condition: (user) => user.culturalLessonsCompleted >= 10
  },
  'pronunciation_perfect': {
    ...
    icon: '🎵',
    xpReward: 40,
    condition: (user) => user.perfectPronunciations >= 5
  },
  'speed_learner': {
    id: 'speed_learner',
    title: 'Speed Learner',
    ...
    icon: '🚀',
    xpReward: 30,
    condition: (user) => user.lessonsToday >= 5
  },
  'persistent_learner': {
    ...
    icon: '💪',
    xpReward: 60,
    condition: (user) => user.daysLearned >= 7
  },
  'shona_scholar': {
    id: 'shona_scholar',
    title: 'Shona Scholar',
    ...
    icon: '🎓',
    xpReward: 200,
    condition: (user) => user.completedLessons >= 25
  }
};
...
function createGamificationConfig() {
  const config = {
    achievements,
    challenges,
    levels: {
      1: { xpRequired: 0, title: 'Beginner', color: 'green' },
      2: { xpRequired: 100, title: 'Novice', color: 'blue' },
      3: { xpRequired: 250, title: 'Apprentice', color: 'purple' },
      4: { xpRequired: 500, title: 'Intermediate', color: 'orange' },
      5: { xpRequired: 1000, title: 'Advanced', color: 'red' },
      6: { xpRequired: 2000, title: 'Expert', color: 'violet' },
      7: { xpRequired: 3500, title: 'Master', color: 'gold' },
      8: { xpRequired: 5000, title: 'Grandmaster', color: 'diamond' }
    },
    rewards: {
      xpMultiplier: 1.0,
      streakBonus: 0.1, // 10% bonus per day in streak
      perfectScoreBonus: 0.2, // 20% bonus for perfect scores
      culturalBonus: 0.15 // 15% bonus for cultural lessons
    }
  };
  
  const configPath = path.join(__dirname, '..', 'lib', 'gamification.ts');
  const configContent = `// Gamification Configuration
export const gamificationConfig = ${JSON.stringify(config, null, 2)};
...
`;
  ...
  console.log('✅ Created gamification configuration');
  console.log('🎮 Added achievements and challenges system');
  console.log('🏆 Gamification features ready!');
}
