...
interface LessonsContent extends ContentBase {
  lessons: Lesson[]
}

interface VocabularyContent extends ContentBase {
  vocabulary: VocabularyItem[]
}

interface ExercisesContent extends ContentBase {
  exercises: Exercise[]
}

interface CulturalNotesContent extends ContentBase {
  notes: CulturalNote[]
  categories: string[]
}
...
type ContentData = LessonsContent | VocabularyContent | ExercisesContent | CulturalNotesContent | AudioContent
...
export async function GET(
  ...
  try {
    ...
    console.error(`Failed to serve content:`, error)
    return NextResponse.json(
...
async function getContent(contentType: string, platform: string): Promise<ContentData | null> {
  switch (contentType) {
    case 'lessons':
...
async function getLessonsContent(platform: string): Promise<LessonsContent> {
  ...
  if (fs.existsSync(enhancedPath)) {
    ...
    return adaptContentForPlatform(content, platform, 'lessons') as LessonsContent
  }
...
async function getVocabularyContent(platform: string): Promise<VocabularyContent> {
  ...
  return adaptContentForPlatform(content, platform, 'vocabulary') as VocabularyContent
}

async function getExercisesContent(platform: string): Promise<ExercisesContent> {
...
async function getCulturalNotesContent(platform: string): Promise<CulturalNotesContent> {
...
async function getAudioContent(platform: string): Promise<AudioContent> {
